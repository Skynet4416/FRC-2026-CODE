...
public class ClimbIOReal implements ClimbIO {
  ...
  private final Debouncer neoLeaderConnectedDebouncer = new Debouncer(0.5, Debouncer.DebounceType.kFalling);
  private final Debouncer neoFollowerConnectedDebouncer = new Debouncer(0.5, Debouncer.DebounceType.kFalling);
  private final Debouncer krakenConnectedDebouncer = new Debouncer(0.5, Debouncer.DebounceType.kFalling);

  private final Alert neoLeaderDisconnectedAlert = new Alert("Climb NEO Leader disconnected!", AlertType.kWarning);
  private final Alert neoFollowerDisconnectedAlert = new Alert("Climb NEO Follower disconnected!", AlertType.kWarning);
  private final Alert krakenDisconnectedAlert = new Alert("Climb Kraken disconnected!", AlertType.kWarning);
  ...
  public ClimbIOReal() {
    ...
    neoConfig.idleMode(IdleMode.kBrake).smartCurrentLimit(Constants.Subsystems.Climber.CurrentLimits.NEO_SUPPLY_LIMIT_AMPS);
    neoConfig.closedLoop.p(Constants.Subsystems.Climber.NeoClosedLoop.KP)
    ...
    krakenConfig.CurrentLimits.StatorCurrentLimit = Constants.Subsystems.Climber.CurrentLimits.KRAKEN_SUPPLY_LIMIT_AMPS;
    krakenConfig.CurrentLimits.StatorCurrentLimitEnable = Constants.Subsystems.Climber.CurrentLimits.STATOR_ENABLED;
    kraken.getConfigurator().apply(krakenConfig);

    solenoid = new DoubleSolenoid(4, PneumaticsModuleType.REVPH, 
  ...
  public void updateInputs(ClimbIOInputs inputs) {
    ...
    inputs.neoPositionRads = Units.rotationsToRadians(neoRotations) / Constants.Subsystems.Climber.NEO_GEAR_RATIO;
    inputs.neoVelocityRadsPerSec = Units.rotationsPerMinuteToRadiansPerSecond(leftNeo.getEncoder().getVelocity()) / Constants.Subsystems.Climber.NEO_GEAR_RATIO;
    ...
    inputs.krakenPositionRads = Units.rotationsToRadians(krakenRotations) / Constants.Subsystems.Climber.KRAKEN_GEAR_RATIO;
    inputs.krakenVelocityRadsPerSec = Units.rotationsToRadians(kraken.getVelocity().getValueAsDouble()) / Constants.Subsystems.Climber.KRAKEN_GEAR_RATIO;
    ...
    inputs.neoLeaderConnected = neoLeaderConnectedDebouncer.calculate(leftNeo.getFirmwareVersion() != 0);
    neoLeaderDisconnectedAlert.set(!inputs.neoLeaderConnected);

    inputs.neoFollowerConnected = neoFollowerConnectedDebouncer.calculate(rightNeo.getFirmwareVersion() != 0);
    neoFollowerDisconnectedAlert.set(!inputs.neoFollowerConnected);

    inputs.krakenConnected = krakenConnectedDebouncer.calculate(kraken.isConnceted());
    krakenDisconnectedAlert.set(!inputs.krakenConnected);
  }

  @Override
  public void setNeoAngle(double rads) {
    double targetRotations = Units.radiansToRotations(rads) * Constants.Subsystems.Climber.NEO_GEAR_RATIO;
    leftNeo.getClosedLoopController().setReference(targetRotations, ControlType.kPosition);
  }

  @Override
  public void setKrakenAngle(double rads) {
    double targetRotations = Units.radiansToRotations(rads) * Constants.Subsystems.Climber.KRAKEN_GEAR_RATIO;
